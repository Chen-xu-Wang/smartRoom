<template>
  ...
  <div ref="root" class="twin-root" :class="{ embedded }" :data-theme="store.theme">
    ...
    <!-- 小区模型与事件按账号裁剪后由后端下发：未登录时不加载任何数据 -->
    <div v-if="gate" class="loading gate">
      <el-icon size="28"><Lock /></el-icon>
      <span>{{ gate }}</span>
      <el-button v-if="!embedded && gateLogin" type="primary" @click="showLogin = true">登录</el-button>
    </div>
    <div v-else-if="!store.ready" class="loading"><el-icon class="is-loading" size="28"><Loading /></el-icon> 正在生成小区模型…</div>
    ...
    <template v-if="store.ready">
      ...
      <div class="left panel scroll">
        ...
        <el-empty v-if="!store.allowedRoles.length" description="当前账号没有 3D 数字孪生的访问权限" />
      </div>
...
import api, { errorText } from '@app/api/index.js'
import { useAuthStore } from '@app/stores/auth.js'
import { Engine } from './engine/Engine'
import { loadBuilding, probeBackend } from './data/loader'
...
const gate = ref('')          // 非空时显示在页面中央：未登录、后端未连接、数据加载失败
const gateLogin = ref(false)  // 是否在提示旁显示登录按钮
let engine = null

const alertHouses = computed(() => store.visibleEvents.filter(e => e.houseId && Number(String(e.houseId).slice(0, -2)) === store.level.floor).map(e => e.houseId))
...
const goHouse = (h) => {
  if (!store.canSeeHouse(h)) { ElMessage.warning(store.deniedText()); return }
...
// 独立运行时令牌失效：整页刷新回到登录入口（内嵌时由前端 App.vue 跳转登录页）
const onAuthExpired = () => { if (!props.embedded) location.reload() }

onMounted(async () => {
  store.setThemeRoot(props.embedded ? root.value : document.documentElement)
  store.applyTheme(store.theme, { engine: false })
  window.addEventListener('auth:expired', onAuthExpired)
  store.backendOnline = await probeBackend()
  if (!useAuthStore().user) {
    gate.value = '请先登录：小区数字孪生只展示当前账号有权查看的住户与问题'
    gateLogin.value = true
    showLogin.value = !props.embedded && store.backendOnline
    return
  }
  try {
    await loadBuilding()
  } catch (e) {
    gate.value = store.backendOnline ? `小区模型加载失败：${errorText(e, '请稍后重试')}` : '后端未连接，无法加载小区模型'
    return
  }
  ...
  // 引擎层的越权拦截（楼层、住户、构件跳转）统一在这里提示
  engine.on('denied', () => ElMessage.warning(store.deniedText()))
  engine.load()
  store.applyTheme(store.theme)
  // 任何新的点击都打断进行中的定位动画（"一键定位"按钮的 click 在 pointerdown 之后触发，不受影响）
  document.addEventListener('pointerdown', onPointerDown, true)
  await store.init()
  if (import.meta.env.DEV) window.__twin = { engine, store }
  // 没有入口参数也要走一遍：确定登录身份并锁定视角
  const search = props.query ? new URLSearchParams(props.query).toString() : location.search
  await store.applyEntryParams(search || '', api)
})
...
.loading.gate { flex-direction: column; gap: 12px; padding: 16px; font-size: 15px; text-align: center; }
